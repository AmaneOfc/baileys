...
const executeWMexQuery = async (variables, queryId, dataPath, query, generateMessageTag) => {
    ...
    if (child && child.content) {
        ...
        let response = dataPath ? (data.data && data.data[dataPath]) : data.data;
    ...
    throw new boom_1.Boom(`Failed to ${action}, unexpected response structure: ${JSON.stringify(result, null, 2)}`, { statusCode: 400, data: result });
};
