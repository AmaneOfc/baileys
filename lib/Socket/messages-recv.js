...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleNewsletterNotification = async (node) => {
        ...
        logger.debug({ from, childTag: child && child.tag }, 'got newsletter notification');
        if (!child) return;
        switch (child.tag) {
            case 'reaction': {
            ...
            case 'view': {
                const viewUpdate = {
                    id: from,
                    server_id: child.attrs.message_id,
                    count: parseInt((child.content && child.content.toString()) || '0', 10)
                };
            ...
            case 'update': {
                ...
                if (settingsNode) {
                    ...
                    if (nameNode && nameNode.content) update.name = nameNode.content.toString();
                    const descriptionNode = (0, WABinary_1.getBinaryNodeChild)(settingsNode, 'description');
                    if (descriptionNode && descriptionNode.content) update.description = descriptionNode.content.toString();
            ...
            case 'message': {
                ...
                if (plaintextNode && plaintextNode.content) {
                    try {
                        const contentBuf = Buffer.isBuffer(plaintextNode.content)
                            ? plaintextNode.content
                            : Buffer.from(plaintextNode.content, 'binary');
                        const messageProto = WAProto_1.proto.Message.decode(contentBuf).toJSON();
            ...
            case 'messages': {
                const message = (0, WABinary_1.getBinaryNodeChild)(child, 'message');
                if (message) {
                    ...
                    if (reactionsList) {
                        ...
                        if (reactions.length === 0) {
                            ev.emit('newsletter.reaction', { id: from, server_id: serverId, reaction: { removed: true } });
                        }
                        reactions.forEach(item => {
                            var _a, _b;
                            ev.emit('newsletter.reaction', { id: from, server_id: serverId, reaction: { code: (_a = item.attrs) === null || _a === void 0 ? void 0 : _a.code, count: +((_b = item.attrs) === null || _b === void 0 ? void 0 : _b.count) } });
                        });
                    }
                    if (viewsList.length) {
                        viewsList.forEach(item => {
                            ev.emit('newsletter.view', { id: from, server_id: serverId, count: +item.attrs.count });
                        });
                    }
                }
                break;
            ...
            default:
                logger.debug({ childTag: child.tag, from }, 'Unknown newsletter notification type');
                break;
        }
    };
    const handleMexNewsletterNotification = async (node) => {
        ...
        let data;
        let operation;
        if (mexNode && mexNode.content) {
            try {
                data = JSON.parse(mexNode.content.toString());
                operation = data && data.operation;
            } catch (error) {
                logger.error({ err: error, node }, 'Failed to parse mex newsletter notification (new format)');
            }
        } else {
            const child = (0, WABinary_1.getAllBinaryNodeChildren)(node)[0];
            operation = child && child.attrs && child.attrs.op_name;
            try {
                const raw = (child && child.content && (_a = child.content) === null || _a === void 0 ? void 0 : _a.toString());
                if (raw) data = JSON.parse(raw);
            } catch (error) {
                logger.error({ err: error, node }, 'Failed to parse mex newsletter notification (legacy format)');
                return;
            }
        }
        if (!data || !operation) {
            logger.debug({ node }, 'Invalid or empty mex newsletter notification');
            return;
        }
        const from = node.attrs.from;
        switch (operation) {
            case Types_1.MexOperations.UPDATE:
            case 'NotificationNewsletterUpdate': {
                ...
                } else if (data.data && data.data[Types_1.XWAPaths.METADATA_UPDATE]) {
                    const contentPath = data.data[Types_1.XWAPaths.METADATA_UPDATE];
                    ev.emit('newsletter-settings.update', { id: from, update: contentPath.thread_metadata && contentPath.thread_metadata.settings });
                }
                break;
            }
            case Types_1.MexOperations.PROMOTE:
            case 'NotificationNewsletterAdminPromote': {
                const updates = data.updates;
                if (Array.isArray(updates)) {
                    for (const update of updates) {
                        if (update.jid && update.user) {
                            ev.emit('newsletter-participants.update', { id: update.jid, author: from, user: update.user, new_role: 'ADMIN', action: 'promote' });
                        }
                    }
                } else if (data.data && data.data[Types_1.XWAPaths.PROMOTE]) {
                    const contentPath = data.data[Types_1.XWAPaths.PROMOTE];
                    ev.emit('newsletter-participants.update', { id: from, author: contentPath.actor && contentPath.actor.pn, user: contentPath.user && contentPath.user.pn, new_role: contentPath.user_new_role, action: 'promote' });
                }
                break;
            }
            case Types_1.MexOperations.DEMOTE:
            case 'NotificationNewsletterAdminDemote': {
                const updates = data.updates;
                if (Array.isArray(updates)) {
                    for (const update of updates) {
                        if (update.jid && update.user) {
                            ev.emit('newsletter-participants.update', { id: update.jid, author: from, user: update.user, new_role: 'MEMBER', action: 'demote' });
                        }
                    }
                } else if (data.data && data.data[Types_1.XWAPaths.DEMOTE]) {
                    const contentPath = data.data[Types_1.XWAPaths.DEMOTE];
                    ev.emit('newsletter-participants.update', { id: from, author: contentPath.actor && contentPath.actor.pn, user: contentPath.user && contentPath.user.pn, new_role: contentPath.user_new_role, action: 'demote' });
                }
                break;
            }
            default:
                logger.debug({ operation, data }, 'Unhandled mex newsletter notification');
                break;
        }
    };
